...
## 3. [요청] reflection·조립을 페르소나 빌드 잡에 연결

BE는 이미 **페르소나 빌드 잡(`persona-build-job`)** 이 있으니, 별도 트리거를 새로 만들 필요 없다. **페르소나를 (재)빌드하는 그 잡에서 아래 2콜만 추가**하면 통찰이 페르소나에 반영된다:

1. 그 대상자의 **저장된 기억 전체**를 모아 `POST /v1/persona/reflection` 호출 → 통찰 목록(`reflections[].insight`) 받음.
2. `POST /v1/persona/assembly` 호출 시 context와 함께 그 통찰을 **`personaInsights`(insight 문자열 목록)**로 실어 보냄 → 페르소나 프롬프트(`instructions`) 받음.
3. 받은 `instructions`를 `PersonaBible`에 저장(기존 흐름 그대로).

이러면 **빌드할 때마다 최신 기억 기반 통찰이 페르소나 성격·가치관에 반영**된다.

- **자동 트리거 별도 구현 불필요**: 기존 빌드 잡(신청 완료·재빌드 시점)에 위 2콜을 끼우는 것으로 충분하다. "importance 누적해서 임계값 넘으면 재빌드" 같은 정교한 자동 재빌드는 원하면 나중에 얹으면 되고, 지금은 필요 없다.
- **녹음이 적어도 무해**: 기억이 1~2건뿐이면 통찰이 거의 안 나오지만(빈 배열), 그래도 `personaInsights=[]`로 `/assembly`가 Intake만으로 정상 조립한다. 즉 빌드 잡에 항상 이 2콜을 넣어둬도 된다.
